...
export class TinaCMSImageOptimizer {
  constructor(baseDirectory = 'public') {
   ...
   */
  async optimizeImage(relativePath) {
   ...
   */
  async optimizeDirectory(directory = 'uploads') {
   ...
   */
  async getImageFilesRecursive(dir) {
    const files = []
   ...
   */
  async deleteOptimizedVersions(relativePath) {
    try {
      const fullPath = path.join(process.cwd(), this.baseDirectory, relativePath)
      const dir = path.dirname(fullPath)
      const optimizedDir = path.join(dir, 'optimized')
